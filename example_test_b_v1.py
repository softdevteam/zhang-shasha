...
def compoundStatementEditor(pythonView, inheritedState, model, compoundBlocks):
    ...
    for i, block in enumerate(compoundBlocks):
        ...
        headerStatementLine = SoftStructuralItem(PythonSyntaxRecognizingController.instance, headerNode,
                                                 headerStatementLine)
        ...
        if suite is not None:
            ...
            suiteElement = SoftStructuralItem(PythonSyntaxRecognizingController.instance,
                                              PythonSyntaxRecognizingController.instance._makeCompoundSuiteEditFilter(
                                                  suite),
...
def _onDrop_embeddedObject(element, pos, data, action):
    ...
    if marker is not None and marker.isValid():
        ...
        menu = VPopupMenu([MenuItem.menuItemWithLabel('Copy', _makeOnDrop(_performDropByCopy)),
                           ...
                           MenuItem.menuItemWithLabel('Reference (as literal)',
                                                      _makeOnDrop(_performDropAsLiteralByRef))])
        menu.popupMenuAtMousePosition(marker.getElement())
    return True


_embeddedObject_dropDest = ObjectDndHandler.DropDest(FragmentData, None, _highlightDrop_embeddedObject,
                                                     _onDrop_embeddedObject)
...
def UnparsedStatement(method):
    def _m(self, fragment, inheritedState, model, *args):
        ...
        v = PythonSyntaxRecognizingController.instance._unparsedStatementEditRule.applyToFragment(statementLine(v),
                                                                                                  model, inheritedState)
...
def TargetTopLevel(method):
    def _m(self, fragment, inheritedState, model, *args):
        v = method(self, fragment, inheritedState, model, *args)
        return PythonSyntaxRecognizingController.instance._targetTopLevelEditRule.applyToFragment(v, model,
                                                                                                  inheritedState)
...
def StructuralExpression(method):
    def _m(self, fragment, inheritedState, model, *args):
        v = method(self, fragment, inheritedState, model, *args)
        return PythonSyntaxRecognizingController.instance._structuralExpressionEditRule.applyToFragment(v, model,
                                                                                                        inheritedState)
...
def ExpressionTopLevel(method):
    def _m(self, fragment, inheritedState, model, *args):
        v = method(self, fragment, inheritedState, model, *args)
        return PythonSyntaxRecognizingController.instance._expressionTopLevelEditRule.applyToFragment(v, model,
                                                                                                      inheritedState)
...
def CompoundStatementHeader(method):
    def _m(self, fragment, inheritedState, model, *args):
        ...
        if isinstance(v, tuple):
            ...
            e = PythonSyntaxRecognizingController.instance._compoundStatementHeaderEditRule.applyToFragment(e, model,
                                                                                                            inheritedState)
            ...
            v = statementLine(v)
            v = v.withProperty(StatementProperty.instance, model)
            v = PythonSyntaxRecognizingController.instance._compoundStatementHeaderEditRule.applyToFragment(v, model,
                                                                                                            inheritedState)
            v = _applyIndentationShortcuts(v)
            return v
...
def SpecialFormStatement(method):
    def _m(self, fragment, inheritedState, model, *args):
        ...
        v = v.withProperty(StatementProperty.instance, model)
        v = PythonSyntaxRecognizingController.instance._specialFormStatementEditRule.applyToFragment(v, model,
                                                                                                     inheritedState)
        v = _applyIndentationShortcuts(v)
        return v
...
class Python2View(MethodDispatchView):
    ...
    def PythonModule(self, fragment, inheritedState, model, suite):
        ...
        s = SoftStructuralItem(PythonSyntaxRecognizingController.instance,
                               [PythonSyntaxRecognizingController.instance._makeSuiteEditFilter(suite),
    ...
    def EmbeddedObjectLiteral(self, fragment, inheritedState, model, embeddedValue):
        ...
        return view.withContextMenuInteractor(_embeddedObjectLiteralContextMenuFactory).withTreeEventListener(
            _EmbeddedObjectExprTreeEventListener.instance)
    ...
    def EmbeddedObjectExpr(self, fragment, inheritedState, model, embeddedValue):
        ...
        return view.withContextMenuInteractor(_embeddedObjectExprContextMenuFactory).withTreeEventListener(
            _EmbeddedObjectExprTreeEventListener.instance)
    ...
    def EmbeddedObjectStmt(self, fragment, inheritedState, model, embeddedValue):
        ...
        if expansionFn is None:
            ...
            return view.withContextMenuInteractor(_embeddedObjectStmtContextMenuFactory).withTreeEventListener(
                _EmbeddedObjectStmtTreeEventListener.instance)
        else:
            # Macro view
            def createExpansionView():
                return Pres.coerce(expansionFn())
    ...
    def DefStmtHeader(self, fragment, inheritedState, model, name, params, paramsTrailingSeparator):
        return self._defStmtHeaderElement(inheritedState, name, params,
                                          paramsTrailingSeparator), defStmtHeaderHighlight, defStmtHighlight
    ...
    def ClassStmtHeader(self, fragment, inheritedState, model, name, bases, basesTrailingSeparator):
        return self._classStmtHeaderElement(inheritedState, name, bases,
                                            basesTrailingSeparator), classStmtHeaderHighlight, classStmtHighlight
    ...
    def IndentedBlock(self, fragment, inheritedState, model, suite):
        ...
        suiteElement = suiteElement.withProperty(SuiteProperty.instance, suite)
        suiteElement = SoftStructuralItem(PythonSyntaxRecognizingController.instance,
                                          PythonSyntaxRecognizingController.instance.parsingEditFilter('Suite',
                                                                                                       _parser.compoundSuite(),
                                                                                                       _makeSuiteCommitFn(
                                                                                                           suite)),
                                          model, suiteElement)
    ...
    def IfStmt(self, fragment, inheritedState, model, condition, suite, elifBlocks, elseSuite):
        compoundBlocks = [
            (Schema.IfStmtHeader(condition=condition), self._ifStmtHeaderElement(inheritedState, condition), suite)]
        for b in elifBlocks:
            if not b.isInstanceOf(Schema.ElifBlock):
                raise TypeError, 'IfStmt elifBlocks should only contain ElifBlock instances'
            compoundBlocks.append((Schema.ElifStmtHeader(condition=b['condition']),
                                   self._elifStmtHeaderElement(inheritedState, b['condition']), b['suite']))
    ...
    def WhileStmt(self, fragment, inheritedState, model, condition, suite, elseSuite):
        compoundBlocks = [(Schema.WhileStmtHeader(condition=condition),
                           self._whileStmtHeaderElement(inheritedState, condition), suite)]
        if elseSuite is not None:
            compoundBlocks.append((Schema.ElseStmtHeader(), self._elseStmtHeaderElement(inheritedState), elseSuite))
        return compoundBlocks
    ...
    def ForStmt(self, fragment, inheritedState, model, target, source, suite, elseSuite):
        compoundBlocks = [(Schema.ForStmtHeader(target=target, source=source),
                           self._forStmtHeaderElement(inheritedState, target, source), suite)]
        if elseSuite is not None:
            compoundBlocks.append((Schema.ElseStmtHeader(), self._elseStmtHeaderElement(inheritedState), elseSuite))
        return compoundBlocks
    ...
    def TryStmt(self, fragment, inheritedState, model, suite, exceptBlocks, elseSuite, finallySuite):
        ...
        for b in exceptBlocks:
            ...
            compoundBlocks.append((Schema.ExceptStmtHeader(exception=b['exception'], target=b['target']),
                                   self._exceptStmtHeaderElement(inheritedState, b['exception'], b['target']),
                                   b['suite']))
        if elseSuite is not None:
            compoundBlocks.append((Schema.ElseStmtHeader(), self._elseStmtHeaderElement(inheritedState), elseSuite))
        if finallySuite is not None:
            compoundBlocks.append(
                (Schema.FinallyStmtHeader(), self._finallyStmtHeaderElement(inheritedState), finallySuite))
        return compoundBlocks
    ...
    def WithStmt(self, fragment, inheritedState, model, contexts, suite):
        return [
            (Schema.WithStmtHeader(contexts=contexts), self._withStmtHeaderElement(inheritedState, contexts), suite)]
    ...
    def DefStmt(self, fragment, inheritedState, model, decorators, name, params, paramsTrailingSeparator, suite):
        ...
        for d in decorators:
            ...
            compoundBlocks.append((Schema.DecoStmtHeader(name=d['name'], args=d['args'],
                                                         argsTrailingSeparator=d['argsTrailingSeparator']),
                                   self._decoStmtHeaderElement(inheritedState, d['name'], d['args'],
                                                               d['argsTrailingSeparator']), None))

        compoundBlocks.append(
            (Schema.DefStmtHeader(name=name, params=params, paramsTrailingSeparator=paramsTrailingSeparator),
    ...
    def ClassStmt(self, fragment, inheritedState, model, decorators, name, bases, basesTrailingSeparator, suite):
        ...
        compoundBlocks.append(
            (Schema.ClassStmtHeader(name=name, bases=bases, basesTrailingSeparator=basesTrailingSeparator),
...
PythonCommands.pythonCommandSet('LarchCore.Languages.Python2',
                                [_multilineStringCommand, _quoteExprCommand, _quoteStmtCommand, _unquoteCommand])
